...
from utils.user_utils import is_email_valid
from repositories.user_repository import UserRepository
from services.auth_service import clear_token, verify_password, set_password, require_permission  # noqa: E501
...
class UserService:
    ...
    def get_user_by_name(self, full_name: str):
        ...
        try:
            ...
            if not existing_user:
                logging.debug("Utilisateur introuvable avec le nom "
                              f"{full_name}")
                return {"error": "Utilisateur introuvable"}
    ...
    def update_user(self, email: str, full_name: str = None,
                    new_email: str = None, password: str = None,
                    role_id: int = None):
        """
        ...
        try:
            existing_user = self.user_repo.get_user_by_email(email)
            if not existing_user:
                logging.debug(f"Utilisateur introuvable avec l'email {email}")
                return {"error": "Utilisateur introuvable"}
            ...
            updated_user = self.user_repo.update_user(
                ...
                full_name=full_name,
                email=new_email if new_email else existing_user.email,
                password=password,
                role_id=role_id
                )
            return updated_user
